...
app.get("/", (_req, res) => {
  res.json({ message: "Daily Telegraph Backend Running" });
});
...
app.get("/api/survey/dashboard", auth, async (req: any, res: any) => {
  try {
...
app.post("/api/survey/complete", auth, async (req: any, res: any) => {
  try {
...
app.post("/api/survey/withdraw", auth, async (req: any, res: any) => {
  try {
    const user: any = await User.findById(req.user.id);
    if (!user) return res.status(404).json({ msg: "User not found" });
